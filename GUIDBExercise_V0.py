...
class MyWindow(tk.Tk):
    ...
    def __init__(self):
        ...
        # TODO: position the Widget
        
        nameE=tk.Entry(self, textvariable=self.nameStr)
        # TODO: position the Widget
        
        # TODO: Create and position 2 widgets for the price (A label + And Entry)
        
        # TODO: Create and position 2 widgets for the qty (A label + And Entry)
        
        loadB=tk.Button(self, text="update", command=self.update)
        loadB.grid(row=2, column=3, padx=10, pady=10)
        
        # TODO: add the 2 other buttons
        
        # the bind() method is used to register an event listener with
        # the event "ListBoxSelect" of the ListBox listB:
        
        # TODO: add a Entry to display the number of row
        # TODO: add a checkbox to activate or deactivate the possibility to 
        # select a row
    ...
    def insert(self):
        ...
        try:
            ...
            cursor=conn.cursor()
            # TODO: Insert a new row in the table product
            cursor.close()
            conn.close()
            self.onLoad()
    ...
    def update(self):
        ...
        try:
            conn=sqlite3.connect(r"epfl.db")
            cursor=conn.cursor()
            # TODO: Update a row of the table product
            cursor.close()
            conn.close()
            self.onLoad() 
        except Exception as ex:
    ...
    def delete(self):
        ...
        try:
            conn=sqlite3.connect(r"epfl.db")
            cursor=conn.cursor()
            # TODO: delete a row of the table product
            cursor.close()
            conn.close()
            self.onLoad() 
        except Exception as ex:
